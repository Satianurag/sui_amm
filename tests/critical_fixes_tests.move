...
module sui_amm::critical_fixes_tests {
    ...
    fun test_partial_removal_retains_fees() {
        ...
        {
            let pool = pool::create_pool<USDT, USDC>(30, 0, 0, ts::ctx(scenario));
            pool::share(pool);
        };
        ...
        ts::next_tx(scenario, ALICE);
        {
            ...
            let (position, r_a, r_b) = pool::add_liquidity(pool, coin_a, coin_b, 0, ts::ctx(scenario));
        ...
        ts::next_tx(scenario, BOB);
        {
            let pool_val = ts::take_shared<LiquidityPool<USDT, USDC>>(scenario);
            let pool = &mut pool_val;
            let coin_in = coin::mint_for_testing<USDT>(100000, ts::ctx(scenario));
            
            // Swap 100k USDT -> USDC. Fee is 0.3% = 300 USDT.
            let coin_out = pool::swap_a_to_b(pool, coin_in, 0, option::none(), &clock, 1000, ts::ctx(scenario));
        ...
        ts::next_tx(scenario, ALICE);
        {
            ...
            let (fee_a, fee_b) = sui_amm::fee_distributor::claim_fees_simple(pool, position, ts::ctx(scenario));
    ...
    fun test_reject_dust_liquidity() {
        ...
        ts::next_tx(scenario, ALICE);
        {
            let pool_val = pool::create_pool<USDT, USDC>(30, 0, 0, ts::ctx(scenario));
            let pool = &mut pool_val;
            
            // Try to add tiny liquidity (sqrt(10*10) = 10 < 1000 MINIMUM)
            let coin_a = coin::mint_for_testing<USDT>(10, ts::ctx(scenario));
            let coin_b = coin::mint_for_testing<USDC>(10, ts::ctx(scenario));
    ...
    fun test_display_setup_works_without_svg() {
        ...
        ts::next_tx(scenario, ALICE);
        {
            let pool_val = pool::create_pool<USDT, USDC>(30, 0, 0, ts::ctx(scenario));
            let pool = &mut pool_val;
            let coin_a = coin::mint_for_testing<USDT>(1_000_000, ts::ctx(scenario));
            let coin_b = coin::mint_for_testing<USDC>(1_000_000, ts::ctx(scenario));

            let (pos, r_a, r_b) = pool::add_liquidity(pool, coin_a, coin_b, 0, ts::ctx(scenario));
    ...
    fun test_stableswap_price_impact_uses_spot_not_cp() {
        ...
        ts::next_tx(scenario, ADMIN);
        {
            let pool_val = stable_pool::create_pool<USDT, USDC>(30, 0, 100, ts::ctx(scenario));
            let pool = &mut pool_val;

            let coin_a = coin::mint_for_testing<USDT>(1_000_000, ts::ctx(scenario));
            let coin_b = coin::mint_for_testing<USDC>(500_000, ts::ctx(scenario));

            let (pos, r_a, r_b) = stable_pool::add_liquidity(pool, coin_a, coin_b, 0, ts::ctx(scenario));
        ...
        ts::next_tx(scenario, ADMIN);
        {
            ...
            let out = stable_pool::swap_a_to_b(pool, coin_in, 0, option::none(), &clock, 10_000, ts::ctx(scenario));
